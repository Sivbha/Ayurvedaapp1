...
export default function PrakritiPhysicalPage() {
  ...
  const questions = (prakritiRules.questions as any).physical;

  useEffect(() => {
    if (!assessmentId) return;
    (async () => {
      try {
        const { data } = await proxyFetch('prakriti_answers', { assessment_id: assessmentId, category: 'physical' });
        if (data) {
        ...
        console.error('Load prakriti answers error:', err);
      }
    })();
  }, [assessmentId]);
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-amber-700">Select the option that best describes you throughout your life (not just currently)</p>
