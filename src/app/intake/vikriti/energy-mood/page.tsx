...
export default function VikritiEnergyMoodPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await proxyFetch('vikriti_answers', { assessment_id: assessmentId, category: 'energy_mood' }, { select: 'question_key, severity' });
        if (data) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-amber-700">Rate your energy and mood symptoms (0 = none, 3 = severe)</p>
      ...
      <div className="flex gap-3">
        <button onClick={() => router.push('/intake/vikriti/digestive')} className="rounded-lg border border-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-50">Previous</button>
        <button onClick={() => router.push('/intake/vikriti/physical')} disabled={!allAnswered}
          className="flex-1 rounded-lg bg-amber-700 px-4 py-2 text-white hover:bg-amber-800 disabled:opacity-50">Continue to Physical</button>
      </div>
    </div>
  );
}
